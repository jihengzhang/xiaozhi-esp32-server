...
export class StreamingContext {
    constructor(opusDecoder, audioContext, sampleRate, channels, minAudioDuration) {
        ...
        this.activeQueue = new BlockingQueue(); // Decoded PCM queue. Preparing to play
        ...
        this.endOfStream = false; // Whether received end signal
    ...
    // Get pending buffer queue, single threaded: no safety issues in state of audioBufferQueue constantly updating
    ...
    // Get playing decoded PCM queue, single threaded: no safety issues in state of activeQueue constantly updating
    ...
    convertInt16ToFloat32(int16Data) {
        ...
        for (let i = 0; i < int16Data.length; i++) {
            // Convert range from [-32768,32767] to [-1,1], use 32768.0 uniformly to avoid asymmetric distortion
    ...
    // Get pending play sample count (convert to packet count, each packet 960 samples)
    getPendingPlayCount() {
    ...
    clearAllBuffers() {
        ...
        // Clear all queues (using clear method to maintain object reference)
    ...
    async startPlaying() {
        ...
        while (true) {
            ...
            while (this.playing && this.queue.length > 0) {
                ...
                log(`Scheduling playback ${currentSamples.length} samples, approx ${(currentSamples.length / this.sampleRate).toFixed(2)} seconds`, 'debug');
                this.source.start(startTime);

                // Update next audio block scheduling time
                const duration = audioBuffer.duration;
                this.scheduledEndTime = startTime + duration;
                this.lastPlayTime = startTime;

                // Wait for new data if not enough in queue
