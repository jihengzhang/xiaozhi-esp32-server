...
export class UIController {
    ...
    updateSessionStatus(isSpeaking) {
        ...
        if (isSpeaking === null) {
            // Offline status
            sessionStatus.innerHTML = bgHtml + '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> HiPanda Offline</span>';
            sessionStatus.className = 'status offline';
        } else if (isSpeaking) {
            // Speaking
            sessionStatus.innerHTML = bgHtml + '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> HiPanda Speaking</span>';
            sessionStatus.className = 'status speaking';
        } else {
            // Listening
            sessionStatus.innerHTML = bgHtml + '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> HiPanda Listening</span>';
    ...
    updateSessionEmotion(emoji) {
        ...
        // Get current text content, extract non-emoji parts
    ...
    updateAudioStats() {
        ...
        // Only display background progress when speaking
        if (sessionStatus && sessionStatus.classList.contains('speaking') && sessionStatusBg) {
            if (stats.pendingPlay > 0) {
                ...
                // Change background color based on buffering amount
                if (stats.pendingPlay < 5) {
                    // Insufficient buffering: orange-red transparent
    ...
    // Start monitoring audio statistics
    ...
    initEventListeners() {
        ...
        toggleButton.addEventListener('click', () => {
            ...
            toggleButton.textContent = this.isEditing ? 'Done' : 'Edit';
        ...
        // Record button - changed to spacebar press/release mechanism
        const recordButton = document.getElementById('recordButton');
        ...
        // Optional: Keep click functionality as fallback
